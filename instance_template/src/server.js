...
// --- Auto-setup env vars (set by the backend orchestrator) ---
// When these are present, the instance auto-configures on first boot without
// requiring the user to visit /setup.
const AUTO_TELEGRAM_TOKEN = process.env.TELEGRAM_TOKEN?.trim();
const AUTO_LLM_PROVIDER = process.env.LLM_PROVIDER?.trim();  // openai | anthropic | deepseek
const AUTO_LLM_API_KEY = process.env.LLM_API_KEY?.trim();
const AUTO_DISCORD_TOKEN = process.env.DISCORD_TOKEN?.trim();
const AUTO_SLACK_BOT_TOKEN = process.env.SLACK_BOT_TOKEN?.trim();
const AUTO_SLACK_APP_TOKEN = process.env.SLACK_APP_TOKEN?.trim();
const AUTO_SYSTEM_PROMPT = process.env.SYSTEM_PROMPT?.trim();
const AUTO_SKILLS_JSON = process.env.SKILLS_JSON?.trim();  // JSON string of skill config

// Backend callback URL — instance POSTs status updates here.
const CALLBACK_URL = process.env.CALLBACK_URL?.trim();

// Backend API base URL for periodic health reporting.
const BACKEND_API_URL = process.env.BACKEND_API_URL?.trim();

// Unique instance/service identifier (set by orchestrator).
...
// ==========================================
// AUTO-SETUP: configure OpenClaw from env vars at boot
// ==========================================

/**
 * Maps LLM_PROVIDER env var to the openclaw auth-choice and API key flag.
 */
function resolveAutoSetupAuth() {
  if (!AUTO_LLM_PROVIDER || !AUTO_LLM_API_KEY) return null;

  const providerMap = {
    openai:     { authChoice: "openai-api-key",     flag: "--openai-api-key" },
    anthropic:  { authChoice: "apiKey",             flag: "--anthropic-api-key" },
    gemini:     { authChoice: "gemini-api-key",     flag: "--gemini-api-key" },
    openrouter: { authChoice: "openrouter-api-key", flag: "--openrouter-api-key" },
  };

  const mapped = providerMap[AUTO_LLM_PROVIDER.toLowerCase()];
  if (mapped) return mapped;

  // deepseek and other OpenAI-compatible providers → custom provider flow
  return { authChoice: null, customProvider: AUTO_LLM_PROVIDER.toLowerCase() };
}

/**
 * Runs auto-setup using env vars.  Called once at boot when not yet configured.
 */
async function autoSetup() {
  const auth = resolveAutoSetupAuth();
  if (!auth) {
    console.log("[auto-setup] skipped: LLM_PROVIDER or LLM_API_KEY not set");
    return false;
  }

  console.log(`[auto-setup] starting (provider=${AUTO_LLM_PROVIDER})...`);
  ...
  if (auth.authChoice) {
    onboardArgs.push("--auth-choice", auth.authChoice);
    if (auth.flag) {
      onboardArgs.push(auth.flag, AUTO_LLM_API_KEY);
    }
  }

  const onboard = await runCmd(OPENCLAW_NODE, clawArgs(onboardArgs));
  console.log(`[auto-setup] onboard exit=${onboard.code}`);

  const ok = onboard.code === 0 && isConfigured();
  if (!ok) {
    console.error("[auto-setup] onboard failed:", onboard.output);
    await notifyCallback("error", { error: "onboard failed", output: onboard.output });
    return false;
  }

  // Post-onboard config (same as /setup/api/run handler)
  await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "gateway.auth.mode", "token"]));
  await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "gateway.auth.token", OPENCLAW_GATEWAY_TOKEN]));
  await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "gateway.remote.token", OPENCLAW_GATEWAY_TOKEN]));
  await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "gateway.bind", "loopback"]));
  ...
  // Custom provider for deepseek etc.
  if (auth.customProvider) {
    const providerConfigs = {
      deepseek: {
        baseUrl: "https://api.deepseek.com/v1",
        api: "openai-completions",
        apiKey: AUTO_LLM_API_KEY,
      },
    };
    const cfg = providerConfigs[auth.customProvider] || {
      baseUrl: `https://api.${auth.customProvider}.com/v1`,
      api: "openai-completions",
      apiKey: AUTO_LLM_API_KEY,
    };

    const providerCfg = { baseUrl: cfg.baseUrl, api: cfg.api, apiKey: cfg.apiKey };
    await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "models.mode", "merge"]));
    await runCmd(
      OPENCLAW_NODE,
      clawArgs(["config", "set", "--json", `models.providers.${auth.customProvider}`, JSON.stringify(providerCfg)]),
    );
    console.log(`[auto-setup] custom provider ${auth.customProvider} configured`);
  }

  // Auto-add Telegram channel
  if (AUTO_TELEGRAM_TOKEN) {
    const tgCfg = {
      enabled: true,
      dmPolicy: "pairing",
      botToken: AUTO_TELEGRAM_TOKEN,
      groupPolicy: "allowlist",
      streamMode: "partial",
    };
    ...
    console.log("[auto-setup] telegram channel configured");
  }

  // Auto-add Discord channel
  if (AUTO_DISCORD_TOKEN) {
    const dcCfg = {
      enabled: true,
      token: AUTO_DISCORD_TOKEN,
      groupPolicy: "allowlist",
      dm: { policy: "pairing" },
    };
    await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "--json", "channels.discord", JSON.stringify(dcCfg)]));
    console.log("[auto-setup] discord channel configured");
  }

  // Auto-add Slack
  if (AUTO_SLACK_BOT_TOKEN || AUTO_SLACK_APP_TOKEN) {
    const slCfg = {
      enabled: true,
      botToken: AUTO_SLACK_BOT_TOKEN || undefined,
      appToken: AUTO_SLACK_APP_TOKEN || undefined,
    };
    await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "--json", "channels.slack", JSON.stringify(slCfg)]));
    console.log("[auto-setup] slack channel configured");
  }

  // Skills injection
  if (AUTO_SKILLS_JSON) {
    try {
      const skills = JSON.parse(AUTO_SKILLS_JSON);
      await applySkills(skills);
      console.log("[auto-setup] skills injected");
    } catch (err) {
      console.warn(`[auto-setup] failed to parse SKILLS_JSON: ${String(err)}`);
    }
  }

  // System prompt injection
  if (AUTO_SYSTEM_PROMPT) {
    const instrPath = path.join(WORKSPACE_DIR, "CLAUDE.md");
    try {
      fs.writeFileSync(instrPath, AUTO_SYSTEM_PROMPT, "utf8");
      console.log("[auto-setup] system prompt written to CLAUDE.md");
    } catch (err) {
      console.warn(`[auto-setup] failed to write system prompt: ${String(err)}`);
    }
  }
  ...
  await notifyCallback("configured", { provider: AUTO_LLM_PROVIDER });
  return true;
}

/**
 * Apply skills config by writing OpenClaw workspace instruction files.
 */
async function applySkills(skills) {
  const instrDir = path.join(WORKSPACE_DIR, "instructions");
  fs.mkdirSync(instrDir, { recursive: true });

  const skillInstructions = {
    emailProcessing: "You can process and compose emails. Help users draft, summarize, and organize their email communications.",
    scheduleManagement: "You can manage schedules and calendars. Help users plan, organize, and track their appointments and tasks.",
    dataAnalysis: "You can analyze data and provide insights. Help users understand trends, patterns, and statistics in their data.",
    reportGeneration: "You can generate reports. Help users create structured, professional reports from available data.",
    taskAutomation: "You can automate repetitive tasks. Help users set up workflows and automate routine operations.",
    customerSupport: "You can handle customer support interactions. Help users respond professionally to customer inquiries and resolve issues.",
  };

  let activeSkills = [];
  for (const [skill, enabled] of Object.entries(skills)) {
    if (enabled && skillInstructions[skill]) {
      activeSkills.push(skillInstructions[skill]);
    }
  }

  if (activeSkills.length > 0) {
    const content = `# Active Skills\n\n${activeSkills.map((s, i) => `${i + 1}. ${s}`).join("\n")}\n`;
    fs.writeFileSync(path.join(instrDir, "skills.md"), content, "utf8");
  }
}


// ==========================================
// WEBHOOK CALLBACK: notify backend on status changes
// ==========================================

async function notifyCallback(status, extra = {}) {
  ...
  try {
    ...
    console.log(`[callback] notified: ${status}`);
  } catch (err) {
    console.warn(`[callback] failed: ${String(err)}`);
  }
}


// ==========================================
// HEALTH MONITORING: periodic metrics reporting
// ==========================================
...
function startHealthReporting() {
  if (!BACKEND_API_URL || !INSTANCE_ID) return;

  bootTime = Date.now();

  healthReportInterval = setInterval(async () => {
    try {
      const gatewayReachable = isConfigured() ? await probeGateway() : false;

      await fetch(`${BACKEND_API_URL}/api/instance-health`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          instanceId: INSTANCE_ID,
...
app.get("/api/logs/stream", requireSetupAuth, (req, res) => {
  res.writeHead(200, {
  ...
  const logProc = childProcess.spawn(OPENCLAW_NODE, clawArgs(["logs", "--tail", "100", "--follow"]), {
    env: {
      ...process.env,
      OPENCLAW_STATE_DIR: STATE_DIR,
      OPENCLAW_WORKSPACE_DIR: WORKSPACE_DIR,
  ...
  logProc.stdout?.on("data", (chunk) => {
    ...
    for (const line of lines) {
      if (line.trim()) {
        res.write(`data: ${JSON.stringify({ type: "log", line: redactSecrets(line) })}\n\n`);
      }
    }
  });
  ...
  logProc.stderr?.on("data", (chunk) => {
    ...
    for (const line of lines) {
      if (line.trim()) {
        res.write(`data: ${JSON.stringify({ type: "error", line: redactSecrets(line) })}\n\n`);
      }
    }
  });
...
// ==========================================
// API: Hot config reload (update without redeploy)
// ==========================================

app.post("/api/config/update", requireSetupAuth, async (req, res) => {
  try {
    const updates = req.body || {};
    const results = [];

    // LLM provider change
    if (updates.llmProvider && updates.llmApiKey) {
      const auth = {
        openai:     { authChoice: "openai-api-key",     flag: "--openai-api-key" },
        anthropic:  { authChoice: "apiKey",             flag: "--anthropic-api-key" },
        gemini:     { authChoice: "gemini-api-key",     flag: "--gemini-api-key" },
      ...
      const mapped = auth[updates.llmProvider];
      if (mapped) {
        // For standard providers, update auth config
        await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "auth.choice", mapped.authChoice]));
        results.push({ field: "llmProvider", ok: true });
      } else {
        // Custom provider (deepseek etc.)
        const providerConfigs = {
          deepseek: { baseUrl: "https://api.deepseek.com/v1", api: "openai-completions" },
        };
        const cfg = providerConfigs[updates.llmProvider] || {
          baseUrl: `https://api.${updates.llmProvider}.com/v1`, api: "openai-completions",
        };
        const providerCfg = { baseUrl: cfg.baseUrl, api: cfg.api, apiKey: updates.llmApiKey };
        await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "models.mode", "merge"]));
        await runCmd(
          OPENCLAW_NODE,
          clawArgs(["config", "set", "--json", `models.providers.${updates.llmProvider}`, JSON.stringify(providerCfg)]),
        );
        results.push({ field: "llmProvider", ok: true, custom: true });
      }
    }
    ...
    if (updates.telegramToken) {
      const tgCfg = {
        enabled: true,
        dmPolicy: "pairing",
        botToken: updates.telegramToken,
        groupPolicy: "allowlist",
        streamMode: "partial",
      };
      await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "--json", "channels.telegram", JSON.stringify(tgCfg)]));
      await runCmd(OPENCLAW_NODE, clawArgs(["plugins", "enable", "telegram"]));
      results.push({ field: "telegramToken", ok: true });
    }

    // Discord token update
    if (updates.discordToken) {
      const dcCfg = {
        enabled: true,
        token: updates.discordToken,
        groupPolicy: "allowlist",
        dm: { policy: "pairing" },
      };
      await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "--json", "channels.discord", JSON.stringify(dcCfg)]));
      results.push({ field: "discordToken", ok: true });
    }

    // Slack tokens update
    if (updates.slackBotToken || updates.slackAppToken) {
      const slCfg = {
        enabled: true,
        botToken: updates.slackBotToken || undefined,
        appToken: updates.slackAppToken || undefined,
      };
      await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "--json", "channels.slack", JSON.stringify(slCfg)]));
      results.push({ field: "slack", ok: true });
    }

    // Skills update
    if (updates.skills) {
      await applySkills(updates.skills);
      results.push({ field: "skills", ok: true });
    }

    // System prompt update
    if (updates.systemPrompt !== undefined) {
      const instrPath = path.join(WORKSPACE_DIR, "CLAUDE.md");
      fs.writeFileSync(instrPath, updates.systemPrompt || "", "utf8");
      results.push({ field: "systemPrompt", ok: true });
    }

    // Restart gateway to apply changes
    if (isConfigured() && results.length > 0) {
      await restartGateway();
    }
...
// ==========================================
// API: Channel management (add/remove without redeploy)
// ==========================================

app.get("/api/channels", requireSetupAuth, async (_req, res) => {
  try {
    const channels = {};
    for (const ch of ["telegram", "discord", "slack"]) {
      const r = await runCmd(OPENCLAW_NODE, clawArgs(["config", "get", `channels.${ch}`]));
      if (r.code === 0 && r.output.trim()) {
        try {
          channels[ch] = { configured: true, config: redactSecrets(r.output.trim()) };
        } catch {
...
app.post("/api/channels/:channel/add", requireSetupAuth, async (req, res) => {
  try {
    const channel = req.params.channel;
    const config = req.body || {};

    if (!["telegram", "discord", "slack"].includes(channel)) {
      return res.status(400).json({ ok: false, error: "Invalid channel. Use: telegram, discord, slack" });
    }

    await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "--json", `channels.${channel}`, JSON.stringify(config)]));
    ...
    res.json({ ok: true, message: `${channel} channel added` });
  } catch (err) {
    res.status(500).json({ ok: false, error: String(err) });
  }
});

app.post("/api/channels/:channel/remove", requireSetupAuth, async (req, res) => {
  try {
    const channel = req.params.channel;
    if (!["telegram", "discord", "slack"].includes(channel)) {
    ...
    await runCmd(OPENCLAW_NODE, clawArgs(["config", "set", "--json", `channels.${channel}`, JSON.stringify(disabledCfg)]));

    if (isConfigured()) {
      await restartGateway();
    }

    res.json({ ok: true, message: `${channel} channel disabled` });
  } catch (err) {
    res.status(500).json({ ok: false, error: String(err) });
  }
});
...
// ==========================================
// API: Instance status (for backend polling)
// ==========================================

app.get("/api/instance-status", async (_req, res) => {
  let gatewayReachable = false;
  if (isConfigured()) {
  ...
  res.json({
    ...
    gatewayRunning: Boolean(gatewayProc),
    gatewayReachable,
    uptimeMs: bootTime ? Date.now() - bootTime : 0,
    lastError: lastGatewayError,
  });
});
...
const server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  // ---- AUTO-SETUP: configure from env vars on first boot ----
  if (!isConfigured() && AUTO_LLM_PROVIDER && AUTO_LLM_API_KEY) {
    console.log("[wrapper] not configured but env vars present — running auto-setup...");
    try {
      const setupOk = await autoSetup();
      if (setupOk) {
        console.log("[wrapper] auto-setup succeeded; gateway should be running");
      } else {
        console.error("[wrapper] auto-setup failed; visit /setup for manual configuration");
      }
    } catch (err) {
      console.error(`[wrapper] auto-setup error: ${String(err)}`);
      await notifyCallback("error", { error: String(err) });
    }
  }
  ...
  if (isConfigured() && !gatewayProc) {
    ...
    try {
      ...
      await notifyCallback("running");
    } catch (err) {
      console.error(`[wrapper] gateway failed to start at boot: ${String(err)}`);
    }
  }

  // Start periodic health reporting to the backend
  bootTime = Date.now();
